...
describe('checkout + coupons', () => {
  it('applies a valid coupon to the breakdown', async () => {
    ...
    const order = await checkout({ lines: [{ sku: 'BOOK', quantity: 2 }], couponCode: 'SAVE10' });
    expect(order.breakdown).toEqual({ subtotalCents: 3000, discountCents: 300, taxCents: 189, totalCents: 2889 });
    expect(order.couponCode).toBe('SAVE10');
  });

  it('ignores expired and unknown coupons (no discount)', async () => {
    const a = await checkout({ lines: [{ sku: 'BOOK', quantity: 1 }], couponCode: 'EXPIRED' });
    expect(a.breakdown.discountCents).toBe(0);
    const b = await checkout({ lines: [{ sku: 'BOOK', quantity: 1 }], couponCode: 'NOPE' });
...
describe('checkout + idempotency', () => {
  it('same key twice -> same order, stock reserved once', async () => {
    const first = await checkout({ lines: [{ sku: 'BOOK', quantity: 2 }], idempotencyKey: 'k1' });
    const second = await checkout({ lines: [{ sku: 'BOOK', quantity: 2 }], idempotencyKey: 'k1' });
  ...
  it('different keys -> different orders', async () => {
    const a = await checkout({ lines: [{ sku: 'BOOK', quantity: 1 }], idempotencyKey: 'kA' });
    const b = await checkout({ lines: [{ sku: 'BOOK', quantity: 1 }], idempotencyKey: 'kB' });
